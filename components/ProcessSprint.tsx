...
import { Target, Database, UserCheck, Zap, Activity, Rocket, ArrowRight } from 'lucide-react';
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="absolute inset-0 border-2 border-tech-accent pointer-events-none opacity-0" 
      />

      <div className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-tech-accent/20 group-hover:border-tech-accent transition-colors duration-500" />
      <div className="absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-tech-accent/20 group-hover:border-tech-accent transition-colors duration-500" />
      
      <div className="flex justify-between items-start">
        <motion.div 
          style={{ scale: useTransform(cardProgress, [-10, 0, 10], [1, 1.2, 1]) }}
          className="text-tech-accent opacity-40 group-hover:opacity-100 transition-all duration-700"
        >
          {step.icon}
        </motion.div>
      ...
      <div className="space-y-6 relative z-10">
        ...
        <p className="text-black/50 text-base leading-relaxed font-light max-w-[95%]">
          {step.detail}
        </p>
      </div>
      ...
      <div className="pt-10 border-t border-dashed border-black/10 flex justify-between items-center text-[8px] font-mono text-black/40 uppercase tracking-[0.4em]">
        <div className="flex items-center gap-3">
          <motion.div 
            ...
            className="w-2 h-2 bg-tech-accent rounded-full shadow-[0_0_8px_rgba(255,51,0,0.5)]" 
          />
          <span>SYNCED</span>
        </div>
...
export const ProcessSprint: React.FC = () => {
  ...
  return (
    <section 
      ref={containerRef} 
      id="process" 
      className="relative h-[1000vh] bg-tech-paper z-30"
    >
      <div className="sticky top-0 h-screen w-full flex flex-col justify-center overflow-hidden">
        ...
        >
          <span className="text-[45rem] font-black text-black tracking-tighter uppercase leading-none select-none italic opacity-30">
        ...
        <div className="absolute top-[65%] left-1/2 -translate-x-1/2 w-full max-w-5xl z-30 pointer-events-none px-12 md:px-0">
          ...
          <div className="relative h-[2px] bg-black w-full mb-12">
            
            {/* Pulsing Energy Trail */}
            <motion.div 
              style={{ width: trackHeadPos }}
              className="absolute inset-y-0 left-0 bg-tech-accent"
            />
                ...
                >
                  <motion.div 
                    style={{ 
                      ...
                      backgroundColor: useTransform(smoothProgress, [nodeStart, nodeMid, nodeEnd], ["#000000", "#FF3300", "#000000"]),
                    }}
                    className="w-3.5 h-3.5 border border-white relative z-10"
                  />
                </div>
              );
            })}
          ...
          {/* PHASE LABELS - First letter orange, rest black */}
          <div className="flex justify-between w-full px-2 mb-16">
            {SPRINT_STEPS.map((step, i) => {
              ...
              return (
                ...
                >
                  <div className="whitespace-nowrap font-mono text-[11px] md:text-xs tracking-[0.4em] font-black uppercase">
          ...
          {/* CENTRAL BOOK CONSULTATION CTA - Flat design, no elevation */}
          <div className="flex justify-center mt-4">
             <motion.button 
               whileHover={{ backgroundColor: "#FF3300" }}
               whileTap={{ scale: 0.98 }}
               className="bg-black text-white px-12 py-5 font-mono text-[11px] uppercase tracking-[0.4em] font-black transition-colors duration-500 pointer-events-auto flex items-center gap-4"
             >
        ...
        <div className="absolute top-12 left-12 md:left-24 z-30 pointer-events-none">
          <div className="flex items-center gap-6 text-tech-accent font-mono text-[11px] uppercase tracking-[1em] mb-4 font-black">
            <motion.div 
              style={{ scaleX: pulseScale }}
              className="h-px w-12 bg-tech-accent origin-left" 
            />
            OUR PROVEN SYSTEM .5
          </div>
          <h2 className="text-7xl md:text-[15rem] font-light tracking-tighter text-black uppercase leading-[0.7] select-none opacity-90">
            THE <span className="italic font-bold text-black/5">SPRINT.</span>
          </h2>
        </div>
        ...
        <div className="absolute bottom-16 left-0 w-full px-12 md:px-24 z-30 pointer-events-none">
          <div className="max-w-7xl mx-auto flex justify-between items-end border-t-2 border-black pt-12">
             <div className="flex flex-col gap-4">
               <div className="flex items-center gap-4">
                 <div className="w-3 h-3 bg-tech-accent rounded-full animate-ping" />
                 <span className="text-black font-mono text-[11px] tracking-widest font-black uppercase">SYSTEM_DEPLOYMENT</span>
               </div>
             ...
             <div className="flex flex-col items-end gap-2 pointer-events-none">
                ...
                <motion.div className="text-8xl font-light text-black tracking-tighter leading-none flex items-baseline">
                   ...
                   <span className="text-2xl text-tech-accent ml-4 font-mono font-black italic">/06</span>
                </motion.div>
             </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 pointer-events-none z-20">
            <div className="absolute top-0 left-0 w-full h-[40vh] bg-gradient-to-b from-tech-paper via-tech-paper/40 to-transparent" />
            <div className="absolute bottom-0 left-0 w-full h-[40vh] bg-gradient-to-t from-tech-paper via-tech-paper/40 to-transparent" />
        </div>
      </div>
    </section>
  );
